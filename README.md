...
### Web3.js 操作智能合約
有了『ABI』與『合約地址』後現在可以透過 Web3.js 來操作合約了，首先透過 `web3.eth.Contract` 來與以太鏈上的智能合約連結，它接收兩個參數，一個是合約的ABI、一個是合約地址，因此我們可以把剛剛所產生的 ABI 與 **Deployed Contracts** 後的合約地址傳入 `web3.eth.Contract` 函式中讓它們綁定。
